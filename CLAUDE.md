...
`3. 연결재무제표 주석` 하나에 들어 있고 소제목이 평문입니다.
후자에서 `section="충당부채"`는 **찾지 못합니다**. 이때는 `section="연결재무제표 주석"`으로
연 뒤 `offset`으로 넘겨가며 찾으세요. 응답이 잘리면 다음 `offset`을 알려주며,
잘렸다는 표시가 없을 때까지 반복하면 전체를 읽을 수 있습니다.
