...
※ 분기 보고서(11013/11012/11014)는 '당기 3개월'과 '당기 누계' 두 열을 함께
  ...
  누계 열에서 직접 차감하지 말 것.
  단, '당기 3개월'이 비어 있는 구간에서만 차감으로 산출한다:
...
※ 단, 이 도구는 accounts와 달리 요청한 1개 연도만 반환한다.
  다년치가 필요하면 연도 수만큼 호출해야 하며, 사전에 호출 수를 알릴 것.
※ 과거 연도는 제공되지 않는다 (실측 기준 2023년 이후만 응답이 있고,
  그 이전은 status 013으로 빈다). 빈 응답이 오면 재시도하지 말 것.
  → 2023년 이전을 포함한 다년 비율이 필요하면, 이 도구 대신
    single_financial_accounts로 원계정을 받아 비율을 계산하는 편이 호출 수가 적다
    (accounts는 1회에 3개 시점을 반환한다).
